...
#        top_scores, top_score_ids = flat_beam_scores.topk(k=self.beam_size, dim=0, largest=True, sorted=True)
        top_scores, top_score_ids=flat_beam_scores.topk(k=self.beam_size, dim=0)
...
#        self.all_attentions.append(prev_attention)
    ...
    def get_new_input(self):
        if len(self.next_ys)>1:
            device=self.next_ys[1].device.index
            device_name='cuda:'+str(device)
            self.next_ys[0]=self.next_ys[0].to(device_name)
            return torch.stack(self.next_ys).transpose(0,1)
        else:
            return self.next_ys[-1].unsqueeze(1)
